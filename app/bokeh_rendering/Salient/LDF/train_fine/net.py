...
def _torch_load_compat(path: str):
    ...
    break loading legacy `.tar` checkpoints and many older state_dict files.
    """
    try:
...
class ResNet(nn.Module):
    ...
    def initialize(self):
        ...
        self.load_state_dict(_torch_load_compat(self.resnet_path), strict=False)
...
class LDF(nn.Module):
    ...
    def initialize(self):
        if self.cfg.snapshot:
            self.load_state_dict(_torch_load_compat(self.cfg.snapshot))
        else:
            weight_init(self)
