...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # Try GPU FFT first, fallback to CPU if cuFFT error occurs
        try:
            ...
            if "cuFFT" in str(e) or "CUFFT" in str(e):
                ...
                x_cpu = x.cpu()
                ffted = torch.fft.rfftn(x_cpu, dim=fft_dim, norm=self.fft_norm)
            else:
                raise
        
        ffted = torch.stack((ffted.real, ffted.imag), dim=-1)
        if use_cpu_fallback:
            ffted = ffted.to(device)
        ...
        # Try GPU IFFT first, fallback to CPU if cuFFT error occurs
        try:
            output = torch.fft.irfftn(ffted, s=ifft_shape_slice, dim=fft_dim, norm=self.fft_norm)
        except RuntimeError as e:
            if "cuFFT" in str(e) or "CUFFT" in str(e):
                # Fallback to CPU IFFT
                ffted_cpu = ffted.cpu()
                output = torch.fft.irfftn(ffted_cpu, s=ifft_shape_slice, dim=fft_dim, norm=self.fft_norm)
