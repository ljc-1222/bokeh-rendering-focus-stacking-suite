"""Reference implementation of the scattering layer in pure PyTorch.

This file lives next to the CUDA extension sources and is primarily useful for
debugging and experimentation. The main pipeline uses `scatter_cuda` instead.
"""
...
def soft_step(a, b):
    """Approximate a differentiable step function for `a > b`."""
...
def scatter(x, lens=21, lens_effects=20):
    """Scatter rendering layer (slow reference).

    Args:
        x: Input tensor shaped `B x 4 x H x W` (RGBD) with relative disparity.
        lens: Odd lens size.
    ...
    # Scattering kernels.
    diskernel = distance_kernel(lens).to(x)
    lens_mask = lens_shape_mask(lens, 'circle').to(x)
    ...
    for bi in tqdm(range(b)):
        ...
        for hi in range(offset, h + offset):
            for wi in range(offset, w + offset):
                rgb_neighbours = paddedx[
                    bi,
                    :3,
                    hi - lens // 2 : hi + lens // 2 + 1,
                    wi - lens // 2 : wi + lens // 2 + 1,
                ]
                dneighbours = dscatter[
                    bi,
                    hi - lens // 2 : hi + lens // 2 + 1,
                    wi - lens // 2 : wi + lens // 2 + 1,
                ]
...
if __name__ == '__main__':
    ...
    plt.imsave(
        'original.png',
        np.clip(test[0].detach().cpu().numpy().transpose(1, 2, 0)[:, :, :3], 0.0, 1.0).copy(order='C'),
    )
    plt.imsave(
        'cpu.png',
        np.clip(test_blur[0].detach().cpu().numpy().transpose(1, 2, 0)[:, :, :3], 0.0, 1.0).copy(order='C'),
    )
    ...
    plt.imsave(
        'gpu.png',
        np.clip(test_blur[0].detach().cpu().numpy().transpose(1, 2, 0), 0.0, 1.0)[:, :, :3].copy(order='C'),
    )
